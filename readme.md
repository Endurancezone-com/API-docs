...
| Country           | Acceptable values are: ```Canada```, ```US```, ```UK```,```ALB```,```MX```,```SA```,```AU```,```NZ```,```AF```,```IN```,```AE```,```AT```,```BE```,```BG```,```CH```,|
|                   | ```DE```,```DK```,```ES```,```FR```,```IE```,```IT```,```LU```,```MC```,|
|                   | ```NL```,```NO```,```SE```,```SG```,```ZA```,```UAE```  *additional countries can be enabled as part of the onboarding				                  |
